...
\end{lstlisting}
See the test case in the next page to check your implementation.
\pagebreak
\begin{lstlisting}[language=FSharp]
...
How is this simpler problem relevant to the original question?

\vspace{0.5cm}

Observation: $\sqrt{600851475143} = 775146.099\ldots \approx 775146$. We make a simple \underline{guess} that the solution to the original question is somewhere between $2$ and $775146$ (although, strictly speaking, this \underline{guess} may not work, as explained in the appendix)

\begin{modQuestion*}
Within the range from $2$ to $775146, \ldots$
\begin{enumerate}
\item $\ldots$ which of these numbers are factors of $600851475143$?
\item Out of those numbers in Step $1$, filter/extract out the prime numbers.
\item Out of those numbers in Step $2$, find the maximum prime number.
\end{enumerate}
\end{modQuestion*}
\begin{lstlisting}[language=FSharp]
...
// Answer: 6857
\end{lstlisting}
Here, we used \texttt{BigInteger} to handle large numbers that cannot be handled by \texttt{int}, but otherwise the last two lines \texttt{|> List.filter (IsPrime)} and \texttt{|> List.max} are probably what you have already implemented before in \texttt{FindLargestPrime}

Remark: Technically speaking, our guess of using the square root may not cover all cases. See the \textbf{Appendix} for more details. i.e. if we change $600851475143$ to another number, our \underline{guess} may not work.

But for this specific question, as asked on the original website, the answer is indeed $6857$.
...
\textbf{Question:} Given an integer $Z$, write a function that finds the largest prime factor of $Z$. e.g. The prime factors of $13195$ are $5, 7, 13, 29$, and so the largest for $13195$ is $29$.

...
mutable, if, then, else, cloud, async, static, use, abstract, interface, inherit, finally },
  morekeywords=[2]{double, float, int, string},
...
// Output:
// The new sentence is: Jack and Mary are friends
\end{lstlisting}
...
// Output: 
// "Drink preference: Jack likes to drink coffee."

printfn "Eating preference: %s" (EatFunction "Jill")
// Output:
// "Eating preference: Jill prefers eating chocolate."
\end{lstlisting}
...
"ERROR!!!"
\end{lstlisting}
Because \texttt{score} is an \texttt{int}, it cannot be the input for the function \texttt{GetCAP} (which accepts only \texttt{string}).

Similarly, the intermediate result from \texttt{GetCAP} is \texttt{float}, which cannot be the input for the function \texttt{GetGrade} (which accepts only \texttt{int})
